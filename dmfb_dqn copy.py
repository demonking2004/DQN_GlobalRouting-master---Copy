...
def main():
    ...
    with open(tex_path, "w") as f:
        ...
        f.write("    \\centering\n")
        f.write("    \\begin{tabular}{c|c|c|c|ccc|c}\n")
        f.write("    \\hline\n")
        f.write("    \\textit{Case} & \\textit{ChipSize} & \\#\\textit{ongoing mixers} & \\textit{\\#obs} & \\multicolumn{3}{c|}{Success Rate} & \\textit{Avg. path length}\\\\\n")
        f.write("    \\cline{5-7}\n")
        f.write("     &  &  &  & max & min & avg & \\\\\n")
        f.write("    \\hline\n")
        for r in rows:
            f.write(f"    {r['case']} & {r['chip']} & {r['mixers']} & {r['n_obstacles']} & "
                   f"{r['pr_max']:.1f} & {r['pr_min']:.1f} & {r['pr_avg']:.1f} & {r['avg_path']}\\\\\n")
        f.write("    \\hline\n")
        f.write("    \\end{tabular}\n")
        f.write("\\end{table}\n")
